...
% =========================================================================
% 2. Data Loading (A_eff only, 3 columns for H = 0.05, 0.1, 0.15 um)
% =========================================================================
csv_path = fullfile(fileparts(mfilename('fullpath')), 'Aeff_data.csv');
...
if isfile(csv_path)
    ...
    error('Could not find Aeff_data.csv. Please ensure it is in the same folder as this script.');
end

% =========================================================================
% 3. Calculate Nonlinear Parameter (Gamma) for each H value
% =========================================================================
...
% =========================================================================
% 4. Plotting (Dual Y-Axis for H = 0.05, 0.1, 0.15 um)
% =========================================================================
figure(1)
clf;
hold on;

% Define beautiful, distinct colors for each H value
color_H05  = [0.0, 0.4, 0.8];  % Cobalt Blue for H = 0.05 um
color_H10  = [0.0, 0.6, 0.5];  % Teal for H = 0.1 um
color_H15  = [0.8, 0.1, 0.2];  % Crimson Red for H = 0.15 um

% --- Left Y-Axis: Gamma ---
yyaxis left
h_gamma1 = plot(lambda_um, gamma(:, 1), 'LineStyle', '--', 'Color', color_H05, 'LineWidth', 2.5);
h_gamma2 = plot(lambda_um, gamma(:, 2), 'LineStyle', '--', 'Color', color_H10, 'LineWidth', 2.5);
h_gamma3 = plot(lambda_um, gamma(:, 3), 'LineStyle', '--', 'Color', color_H15, 'LineWidth', 2.5);
...
% --- Right Y-Axis: Aeff ---
yyaxis right
h_Aeff1 = plot(lambda_um, A_eff_um2(:, 1), 'LineStyle', '-', 'Color', color_H05, 'LineWidth', 2.5);
h_Aeff2 = plot(lambda_um, A_eff_um2(:, 2), 'LineStyle', '-', 'Color', color_H10, 'LineWidth', 2.5);
h_Aeff3 = plot(lambda_um, A_eff_um2(:, 3), 'LineStyle', '-', 'Color', color_H15, 'LineWidth', 2.5);
...
legend([h_Aeff1, h_Aeff2, h_Aeff3], ...
    {'A_{eff}, \gamma, H=0.05 \mum', 'A_{eff}, \gamma, H=0.1 \mum', 'A_{eff}, \gamma, H=0.15 \mum'}, ...
    'FontSize', 14, 'FontWeight', 'bold', 'Location', 'north');
legend('boxoff');

% --- Bottom Text Annotation ---
text(0.5, 0.6, 'R = 2.5 \mum', 'Units', 'normalized', 'FontSize', 16, ...
    'FontWeight', 'bold', 'HorizontalAlignment', 'center');

hold off;

% Save the plot
output_image_path = fullfile(fileparts(mfilename('fullpath')), 'Gamma_Aeff_plot.png');
