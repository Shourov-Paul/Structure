...
csv_path = fullfile(fileparts(mfilename('fullpath')), 'All_n_Confloss_H0.1(um)_rearranged.csv');
c = readcell(csv_path);
...
xlim([8, 12]);
ylim([0, 75]); % Fits the curves beautifully
...
legend(handles, 'R = 2 \mum', 'R = 2.25 \mum', 'R = 2.5 \mum', ...
    'FontSize', 18, 'FontWeight', 'bold', 'FontAngle', 'italic', 'Location', 'southwest');
legend('boxoff');

% Text annotation at appropriate scale (Y = 68) in the empty top-left region
text(8.2, 68, 'H = 0.1 \mum', 'FontSize', 18, 'FontWeight', 'bold', 'FontAngle', 'italic');
hold off;

% Save the plot to Conf_Loss directory
output_image_path = fullfile(fileparts(mfilename('fullpath')), 'loss_h=0.1.png');
