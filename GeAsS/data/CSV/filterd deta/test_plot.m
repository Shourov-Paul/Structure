...
lambda = (1000*1e-12:del_lambda:8000*1e-12);
N = length(lambda);
csv_path = fullfile(fileparts(mfilename('fullpath')), 'all_n_RV.csv');
...
plot([1.5 8], [0 0], 'Color', [0.5 0.5 0.5], 'LineStyle', '--', 'linewidth', 1.5);
%plot([4.3 4.3], [-500 200], 'Color', [0.5 0.5 0.5], 'LineStyle', '--', 'linewidth', 1.5);
...
legend([h1, h2, h3, h4], 'R = 2 \mum', 'R = 3 \mum', 'R = 4 \mum', 'R = 5 \mum', ...
    'FontSize', 18, 'FontWeight', 'bold', 'FontAngle', 'italic', 'Location', 'southeast');
legend('boxoff');

text(1.6, 150, 'H = 0.05 \mum', 'FontSize', 18, 'FontWeight', 'bold', 'FontAngle', 'italic');
hold off;
